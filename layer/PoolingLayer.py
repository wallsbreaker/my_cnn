        ...
        for channel in xrange(post_data_channel):
            for row in xrange(post_data_height):
                for col in xrange(post_data_width):
                    ...
                    post_data_tensor[channel, row, col] = pooing_result#self._coefficient[channel] * pooing_result + self._bias[channel]
        post_data_tensor = self._activation.apply_activate_elementwise(post_data_tensor)
        self._post_data.set_data(post_data_tensor)

    def backward(self):
        #update sensitivity
        input_data = self._pre_data.get_data()
        ...
        #for channel in post_channel:
            ...
            #self._coefficient -= self._learning_rate * np.sum(np.multiply(post_data, post_sensitivity[channel]))